...
输入目的地和旅行偏好，即可生成可直接照着走的旅行攻略。默认标准版侧重速度和关键信息；专业版保留更完整的实时数据与详细板块，并在发布前自动审核、诊断和修复主要问题。两种模式均支持 HTML / PDF / DOCX 下载，并在报告中显示本次生成耗时。

## 功能

- **对话式输入** — 自然语言描述需求，无需填表单。如"武汉出发自驾西藏15天，2人，预算15000元"
- **双生成模式** — 默认标准版，压缩实时数据并生成精简完整报告；可切换专业版，保留更丰富的数据和详细板块
- **专业版发布前审核** — 初稿不立即展示；独立调用 `LLM_MODEL` 检查路线折返、重点遗漏、停留不足和前后矛盾，发现主要问题时自动修复一次，再经程序硬校验后发布最终稿
- **多源数据** — 携程问道查询机票/酒店/景点门票，高德补充定位/POI/天气/路线，12306 补充火车余票参考，统一注入 LLM 上下文
- **自驾路线校验** — 国内多点自驾先按地图距离生成路线骨架和逐日节奏，并施加景观道路、门户支线和最低游览时长等语义约束；给定天数无法安全执行时会在生成正文前明确拦截
- **境内外分流** — 出境及港澳台行程跳过中国大陆 12306 与高德驾车规划，避免无效查询和错误路线
- **结构化输出** — 概览统计、天气穿搭、交通、住宿、逐日行程表、预算拆解、预约清单、避坑提示、行前物品、知识图谱
- **完整性保护** — 模型正文为空时自动重试，内容截断时自动续写，并校验逐日行程、行前物品清单和知识图谱等必要板块
...
专业版的第一稿只在服务端内存中暂存，不会提前展示或写入报告库。审核使用与正文相同的 `LLM_MODEL`，但以独立请求执行；最终只保存并展示通过程序终检的报告。标准版不会进入审核和修复流程，因此其耗时不受这些配置影响。

| 环境变量 | 默认值 | 说明 |
|---|---:|---|
| `PRO_REVIEW_MODE` | `repair` | `off` 关闭；`shadow` 诊断后仍发布初稿；`audit` 对需要重写或重规划的主要/严重问题拦截；`repair` 对可安全修复的问题自动重写一次 |
| `PRO_REVIEW_TIMEOUT` | `60` | 单次审核模型请求的超时秒数，最低 10 秒 |
| `PRO_REVIEW_MAX_TOKENS` | `2500` | 结构化诊断结果的最大输出 token，最低 256 |
| `PRO_REVIEW_TOTAL_TIMEOUT` | `420` | 专业版隐藏初稿、审核和修复整条发布流水线的总时限（秒），不会小于单次审核时限 |
| `PRO_REWRITE_MAX_ATTEMPTS` | `1` | 自动重写次数；当前实现限制为 `0` 或 `1`，避免循环重写 |

默认 `repair` 是正式工作模式。需要快速回退时，只需在服务器 `.env` 中设置 `PRO_REVIEW_MODE=off` 并重启服务；首次上线或更换模型时可先使用 `shadow` 收集诊断指标，再切回 `repair`。

无法仅凭当前数据确认的实时开放、票价等问题会标记为 `manual_verify`，保留报告中的官方核实提示后发布，不会让模型虚构确定答案。若审核判断程序锁定的路线骨架本身需要重规划，则本次停止发布，避免靠文字改写掩盖底层路线错误。
...
| `POST /api/generate` | 生成攻略（SSE 流式），请求体 `{"query":"...","mode":"standard"}`；`mode` 支持 `standard`（默认）或 `professional`，需求最多 2000 字 |
...
FastAPI 服务 (app.py)
    ...
    │   ├── trip_intent.py       ← 境内外及自驾意图识别
    │   ├── route_planner.py     ← POI 定位、距离矩阵、路线骨架与逐日节奏
    │   ├── route_rules.py       ← 景观走廊、门户支线与必游停留规则
    │   ├── report_quality.py    ← 专业版程序校验、结构化诊断与修复决策
...
识别境内外、自驾类型并并行采集适用数据
  ...
  └── 高德位置与周边数据（中国大陆行程）
    │
    ▼
国内多点自驾：POI 校验 → 驾车距离矩阵 → 路线语义约束 → 逐日骨架
  └── 天数无法安全容纳路线时停止生成并返回明确原因
    │
    ▼
按模式压缩实时数据并注入锁定骨架 → DeepSeek 生成 Markdown
    │
    ▼
空正文重试 / 截断续写 / 报告完整性校验
    │
    ▼
专业版：隐藏初稿 → 程序预检 → LLM 独立诊断
  ├── 通过或仅轻微问题：保留初稿
  ├── 主要/严重问题：按诊断修复一次
  └── 最终执行 Day、路线、必游地点与板块完整性硬校验
标准版：跳过上述审核与修复步骤
    │
    ▼
Markdown 解析 → 写入生成耗时 → Jinja2 渲染精美 HTML
    │
    ▼
...
测试覆盖生成接口、标准版/专业版切换、专业版隐藏初稿、结构化诊断、条件修复与失败降级、SSE 结果、模型重试与续写、报告完整性、境内外分流、自驾路线与语义约束、不可行路线拦截、多人并发上限、报告隔离、SQLite 并发写入以及 HTML/PDF/DOCX 相关逻辑；测试数据库使用临时目录，不会修改本地 `travel_data.db`。

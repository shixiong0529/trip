...
输入目的地和旅行偏好，即可生成可直接照着走的旅行攻略。默认标准版侧重速度和关键信息，专业版保留更完整的实时数据与详细板块；两种模式均支持 HTML / PDF / DOCX 下载，并在报告中显示本次生成耗时。
...
测试覆盖生成接口、标准版/专业版切换、SSE 结果、模型重试与续写、报告完整性、境内外分流、自驾路线与语义约束、不可行路线拦截、多人并发上限、报告隔离、SQLite 并发写入以及 HTML/PDF/DOCX 相关逻辑；测试数据库使用临时目录，不会修改本地 `travel_data.db`。
